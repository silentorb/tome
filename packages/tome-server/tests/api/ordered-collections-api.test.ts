...
describe("ordered-collections API", () => {
  ...
  test("GET node with tab returns ordered-collection section", async () => {
    ...
    const section = payload.node.sections.find((entry) => entry.type === "ordered-collection");
    expect(section?.configId).toBe("scenes-by-book");
  });

  test("PATCH move reorders scenes", async () => {
    const res = await api.handler(
      new Request("http://127.0.0.1/api/ordered-collections/scenes-by-book/move", {
        method: "PATCH",
