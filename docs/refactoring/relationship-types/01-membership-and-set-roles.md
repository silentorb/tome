...
Registry entry in Marloth `relationship-types.json`:
...
[`packages/tome-db/src/ordered-associations-config/ordered-associations-file.ts`](../../../packages/tome-db/src/ordered-associations-config/ordered-associations-file.ts) — parser requires `membershipEdgeType === "member_of"`.

Marloth `ordered-associations.json` sets `"membershipEdgeType": "member_of"` for `scenes-by-book`.
...
| File | Usage |
| --- | --- |
| `relationship-types.json` | Registers `member_of` composite and perspectives |
...
| **B6** | Membership is the first branch in `resolveCompositeTypeForLink` (step 0). |
