...
Presentation logic branches on relationship type names and includes-classification helpers. Labels can come from `relationship-types.json` `perspectiveLabels` or fall back to formatted slug titles.
...
[`packages/tome-db/src/relationship-type-label.ts`](../../../packages/tome-db/src/relationship-type-label.ts)
...
| File | Usage |
| --- | --- |
| `relationship-types.json` | `perspectiveLabels` for section titles and link-add copy |
