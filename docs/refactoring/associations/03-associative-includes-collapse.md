...
From the member endpoint, both sides project as `includes` (registered as a symmetric composite in `associations.json`).
...
| File | Usage |
| --- | --- |
| `associations.json` | Registers symmetric `includes` composite |
...
Most includes-slug perspectives are **not** separate keys in `associations.json`—only the `includes` composite is registered.
