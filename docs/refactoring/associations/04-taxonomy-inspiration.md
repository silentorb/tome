...
These are **not** collapsed into the generic `includes` bucket (B3). They are registered bidirectional composites in `associations.json` with asymmetric perspectives (taxonomy on one side, `inspirations` on the other).
...
Marloth registry examples (`associations.json`):
...
| File | Usage |
| --- | --- |
| `associations.json` | One composite entry per taxonomy↔inspiration pair |
