...
Marloth `associations.json` still registers **some** specific composites that overlap conceptually with the migration set (e.g. `scenes_product`, `scenes_part`) for ordered collections and table-schema inverses—these are **active** types, not residue.
