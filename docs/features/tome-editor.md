...
- A **global search** widget **must** let users find and open any node via `GET /api/nodes/search` (pluggable **searcher** extension — see [search.md](./search.md)). Each result **should** be an `<a href="?node=…">` (behavior parity per **Cross-linking** above). When `searchAvailable` is false and the query is non-empty, show that search is not configured. Empty query browses nodes by title ascending. Typed-query ordering and ranking come from the active searcher (FTS5 by default in workbench; LIKE is SQL title-then-body order). Body hits may include a `matchPreview` excerpt. Open via sidebar **Search** or **Ctrl/Cmd+K**. Enter activates the highlighted result; Ctrl/Cmd+Enter opens in a new tab. Client-only relevance sorting still applies to association-type pickers and inline table name filters (not the searcher path).
...
```
@ query → GET /api/nodes/search?q=… → title summaries (searcher order; empty q = title browse)
Global search (Ctrl/Cmd+K) → same endpoint; unified title+body search with title-first ranking; empty query lists recent nodes by title
```
