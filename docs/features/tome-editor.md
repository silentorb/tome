...
When a real anchor is impractical (e.g. **reorderable quick links** use `<button>` + `navigateQuickLinkPointerUp`), **emulate** hard-open gestures (`openStandaloneNodeInNewTab` / `openStandaloneNodeInNewWindow`). Do not invent a full custom context menu unless an anchor cannot be used.
