...
**Ordered collections** are graph relationships whose sequence matters. The workspace manages order automatically through dedicated UI—users never edit an `order` field directly. The first configured instance is **Scenes** on the Scenes database node page: scenes are ordered within each book (Product), grouped by Part for display, and reordered via drag-and-drop.
...
- The Scenes database **Ordered members** section (`views.json` → `association: "ordered_members"`) **must** replace the flat database table with an ordered-collection view.
...
- **Authoritative:** graph `order` from ordered-collection edits and direct DB updates. Preserve `order` when mining export data into existing rows.

## Design rationale

### Hidden automatic order

Legacy tooling required manual juggling of an Order column. Ordered collections move sequencing into first-class tooling: drag-and-drop reflects author intent without exposing implementation details.
...
Scenes are the only known use case. Config lives in git-tracked JSON; the engine in `packages/tome-db/src/ordered-collections.ts` stays domain-agnostic. See [session 03](../refactoring/03-ordered-collections-json.md) for the file schema.

## Behavior / pipeline

```
User drag-drop (webview)
  → PATCH /api/ordered-collections/scenes-by-book/move
  → applyOrderedCollectionMove (tome-db)
...
GET /api/nodes/:scenesDbId?scope=:productId
  ...
  → ordered-collection section (book tabs + part groups)
```

## Out of scope (v1)

- UI for registering new ordered-collection configs
...
- `bun test packages/tome-db/tests` — ordered-collection query and move tests
...
| --- | --- |
| `content/model/ordered-collections.json` | Ordered-association config entries (provider ids, composites, column rules) |
| `packages/tome-db/src/ordered-collections-config/` | Parse/load/invalidate config file |
| `packages/tome-db/src/ordered-collections.ts` | View query, move mutation, schema-driven column defs |
| `packages/tome-db/src/database-column-defs.ts` | Shared column-def builder used by database and ordered-collection views |
| `packages/tome-db/src/node-page-sections.ts` | Emits `ordered-collection` section for configured databases |
| `packages/tome-http/` (HTTP handler) | PATCH move endpoint, `scope` query param |
| `packages/tome-editor/src/webview/components/OrderedCollectionView.tsx` | Book tabs + DnD part tables |
