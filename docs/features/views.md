...
**Uniform window pipeline:** table builders call `explodeTableWindowRequest` once (`backend: sql | js`, optional `searchQuery` from trimmed `q`, sorts, limit/offset). Search is an **operator**, not a window mode. Pipeline: explode → optional prior searcher query → uniform window → hydrate. When `searchQuery` is present, run scoped `TomeSearch.searchWindow` (rank + shelf), then hydrate via `listMemberPage({ memberIds })` or relation `…ForTargetIds` — view sorts do not drive that page (no hybrids). When absent and `backend === sql`, run the SQL window with view sorts + limit/offset. Never SQL-`LIMIT` then sort/filter in JS. Flatfile (`backend === js`) remains exempt (`applyNameFilterAndWindow`).
...
**Table `q` (search operator):** editor table name filter goes through the active [`TomeSearch`](./search.md) via `searchWindow` scoped with `allowedNodeIds` (set members / related nodes / composed scope). Ranking and pagination (`total` / `offset` / `limit`) come from the searcher (FTS or LIKE). Dyn and relation **display** still hydrate only the returned window. No searcher → empty window (`total: 0`), matching global search unavailability. Sidebar **Recent** (`recentNodesGraph` / `GET /api/nodes/recent`) is a separate Imp→SQL chrome query — not part of this pipeline.
...
**Coverage today:** relation table sections, **Items / database custom views**, and **composed / generated presentations** share the uniform pipeline. Items and composed use `listMemberPage` for the SQL window (and `memberIds` hydrate after search). Relation sections use `listRelationshipsFromSourceWindow` when there is no search query, and `listRelationshipsFromSourceForTargetIds` after search. Flatfile remains on the legacy name-filter path. Composition layers only fill optional request fields (scope, groups, sorts); plain Items and composed share the same cache contract.
