...
- Supported path ops: Imp `traverse` (single hop via `relationship_projections`; Imp inputs are separate bare `association` + `direction` — Tome packs them to projection types only at SQL/execute time)
- **Ordinary hop authoring:** traverse nodes use ontology tokens (type-table + relation column key) that write ULID + direction into those ports. Free-text association ULIDs are not the default authoring surface when path-hop options load (`listPathHopOptions` invoke). Multi-hop = chain `traverse` nodes, each filled via tokens.
...
- Imp [sql.md](../../../imp-ts/docs/features/sql.md) / [collection-transforms.md](../../../imp-spec/docs/packages/imp-collection-transforms/collection-transforms.md) / [pathing.md](../../../imp-spec/docs/packages/imp-pathing/pathing.md)
